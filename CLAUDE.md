...
4. Schema-per-tenant: SET search_path TO {tenant_id} di awal request
5. Idempotency key wajib untuk semua payment endpoint
6. Timezone: Asia/Jakarta, store UTC di DB
...
12. JWT: httpOnly cookie (web), secure storage (Flutter)
13. OTP expire 5 menit, max 3x resend per 15 menit
14. APK hosted di Cloudflare R2, cek versi setiap app dibuka
...
19. Simple stock Starter = transaction-first — stok deduct otomatis dari transaksi, restock manual hanya saat terima barang, bukan input harian bebas
...
26. Starter + rutin task selalu Haiku — Sonnet hanya Pro+ untuk task kompleks
27. 3 optimasi wajib AI: batching (1 jam), cache (sampai 00.00), compress context (agregat bukan raw)
...
30. Optimistic lock: UPDATE ... WHERE row_version = :expected — kalau rows_affected=0 → retry max 3x → error
31. Payment endpoint WAJIB SELECT FOR UPDATE — tidak boleh ada concurrent payment processing
32. Redis distributed lock = DEFER — row_version cukup sampai ada evidence contention tinggi (>500 outlet aktif)
...
44. midtrans_raw (JSONB) WAJIB disimpan — untuk debug kalau ada dispute
45. pg_dump ke R2 WAJIB jalan sebelum pilot — cron tiap 6 jam
46. Backup testing tiap 2 minggu — catat di MEMORY.md
47. CHECK (stock_qty >= 0) dan CHECK (computed_stock >= 0) — DB level constraint wajib
...
53. Data retention setelah suspend: 60 hari + 90 hari backup R2
