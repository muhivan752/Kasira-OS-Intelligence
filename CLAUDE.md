...
46. **OTP lewat Sefrekuensi = jalur TAMBAHAN, WA tetap jaring (4 Sep 2026).** `backend/services/sefrekuensi.py` + `auth.py:/otp/send`. Kode tetap dibikin & diverifikasi Selaris; Sefrekuensi cuma KURIR (DM Yasmin + push FCM). Respons `/otp/send` sekarang bawa `channel`: `sefrekuensi` atau `whatsapp`.
    - **Jangan panggil `/cek` sebelum kirim.** `/kirim` udah balik 404 sendiri kalau nomornya nggak kedaftar. `check()` itu buat MEMUTUSKAN TAMPILAN (mau nampilin tawaran/pintu apa nggak), bukan buat ngirim. Dua kali jalan bolak balik = OTP telat tanpa alasan.
    - **OTP BUKAN mesin akuisisi.** Buat nerima kode di Sefrekuensi, app-nya harus udah kepasang. Ini hadiah buat yang udah punya + nurunin ongkos Fonnte. Ajakan pasang ditaruh di titik sakit yang lain (notif pesanan masuk), jangan di layar OTP.
    - Gagal apa pun = jatuh ke WA, dan `sefrekuensi.py` nggak pernah melempar exception ke pemanggil. Env kosong = fitur mati total, perilaku balik persis seperti sebelumnya.
    - `SEFREKUENSI_API_URL` + `SEFREKUENSI_PARTNER_KEY` di `.env` (env baru = `compose build backend && up -d --no-deps backend`, `docker restart` nggak reload env). **Recreate container = berkas `docker cp` hilang**, jadi `version.json` wajib dipasang ulang sesudahnya (gotcha #9).
