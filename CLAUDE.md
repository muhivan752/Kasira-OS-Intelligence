...
48. **Delivery gelombang 1: ongkir + jam buka beneran (4 Sep 2026, mig 106).** Sebelumnya alamat antar diterima, jarak dihitung, radius ditegakkan, tapi ongkir nol dan `opening_hours` cuma teks bebas.
    - **Ongkir DI LUAR `orders.total_amount`.** `total_amount` = penjualan (belasan modul laporan, laba rugi, margin membacanya begitu). Ongkir di `orders.delivery_fee`; yang ditagih ke pelanggan = `Order.grand_total` (= `delivery_service.grand_total`) = `Payment.amount_due`. Arus kas (dari payments) otomatis memuat ongkir, laba rugi (dari total_amount) tidak: ongkir dianggap uang lewat ke kurir, bukan pendapatan. Kalau nanti mau dipisah jadi baris "ongkir terkumpul", ambil dari `orders.delivery_fee`, jangan ubah `total_amount`.
    - **Rumus satu tempat `backend/services/delivery_service.py`**: `base + per_km × ceil(km di atas free_km)`, dibulatkan KE ATAS ke Rp 500. Tanpa koordinat = base saja. Server hitung ulang saat order (`connect.py` POST order), angka dari klien cuma buat tampilan. Kuotasi: `geo/place` (ikut alamat) dan `GET /connect/{slug}/delivery-quote?lat&lng`. Tarif per outlet: `delivery_enabled/fee_base/fee_per_km/free_km/min_order` (+ `delivery_radius_km` sekarang bisa lewat `PUT /outlets/{id}`). Minimal order dicek SESUDAH subtotal (400 dengan angka).
    - **COD = tunai antar lahir `pending`**, ditandai lunas di `PUT /orders/{id}/status completed` (orders.py, cabang `is_storefront`). Tunai ambil sendiri TETAP dicatat lunas saat dibuat (diterima kasir saat serah terima). Laporan cuma menghitung order lunas, jadi COD baru masuk laporan saat diantar. Janitor "toko diam" tetap pakai `created_at` buat tunai.
    - **Jam buka `outlets.business_hours` JSONB** `{mon: [["08:00","22:00"]], ...}` + `hours_mode` `manual|schedule`. `is_open` TETAP saklar induk (tutup mendadak). **Satu pintu "toko lagi buka?": `business_hours.effective_open(outlet)`**, dipakai payload storefront (`is_open` = efektif, `is_open_switch` = saklar mentah, `hours_today`, `next_open`), POST order (400 "Toko sedang tutup. Buka lagi besok 08:00."), booking, direktori publik. Tutup < buka = lewat tengah malam, dicek juga dari hari sebelumnya. Zona waktu `outlets.timezone`. `validate()` di `PUT /outlets/{id}` (400 dengan nama hari). JSON-LD pakai `openingHoursSpecification` kalau mode schedule, teks lama kalau manual.
    - **Web**: `components/dashboard/delivery-hours-settings.tsx` (dua kartu di Pengaturan: tarif + jadwal, satu rentang per hari di UI, server dukung lebih). Keranjang nampilin Pesanan / Ongkir / Total, hint "Ongkir mulai Rp X" di pilihan Antar, minimal order sebagai validasi. Halaman lacak dan WA pakai `grand_total`.
    - **App (nebeng APK)**: kartu Pesanan Online nampilin Pesanan / Ongkir / "Tagih ke pelanggan" buat COD; struk (`ReceiptData.deliveryFee`) baris Ongkir sebelum TOTAL, cuma di pembangun struk order utama.
    - **Pin toko WAJIB di tokonya, dan GPS app kasir NGGAK bisa dipercaya buat itu** (kegigit Ivan 4 Sep malam): `POST /outlets/{id}/location` dari app nyimpen posisi HP saat itu (rumah pemilik, Komplek J City), bukan kafe di Jl. Sudirman 4,7 km jauhnya, jadi semua alamat dekat kafe kebaca 4 sampai 6 km dan ditolak. Sekarang ada blok **Titik lokasi toko** di kartu Antar (Pengaturan web): cari lewat Google Places (proxy `/connect/{slug}/geo/*`, kunci nggak ke browser), pratinjau `GET /connect/geo/static`, simpan lewat `setOutletLocation` → `/outlets/{id}/location`, tombol "Pakai alamat toko" = geocode `outlet.address`. Kartu tarif juga dulu ngereset angka yang lagi diketik tiap `setOutlet` dari kartu lain (jangkauan 5 jadi 1); init sekarang cuma sekali per `outlet.id` (`initFor` ref). Kalau ada kartu Pengaturan baru yang punya input diketik, pakai pola yang sama.
    - **Bayar sekarang vs bayar di tempat itu KEPUTUSAN yang harus kelihatan** (Ivan 4 Sep malam, mig 107). Di keranjang, Antar punya dua kelompok: **Bayar sekarang** (QRIS, dan Transfer bank kalau `payment_methods` punya `transfer` + `bank_account_number` terisi) dan **Bayar di tempat (COD)** (tunai). `outlets.delivery_cod_enabled` (saklar di kartu Antar, default true) false = kelompok COD hilang dan server 400 buat tunai+antar. Transfer = metode manual ketiga di storefront (`connect.py` POST order terima `transfer`): Payment `pending` channel `manual`, halaman lacak nampilin rekening + unggah bukti (blok yang sama dengan QRIS statis), `accept_order` menandai lunas buat qris DAN transfer manual, `/orders/online` nggak menyaringnya. Pesan WA pakai `manual_method` buat nyebut "rekening toko" vs "QRIS toko". Urutan tetap: order dibuat dulu (pending), baru bayar; jangan dibalik, order-nya yang nentuin nominal dan idempotensi.
    - **BELUM**: kurir sebagai entitas, status "sedang diantar" terpisah dari `ready`, bukti serah terima, lacak posisi, rentang jam kedua di UI. Itu gelombang 2 dan seterusnya.

49. **Delivery gelombang 2: kurir = ORANG TOKO (5 Sep 2026, mig 108).** Keputusan Ivan: yang nganter itu anak pemilik, karyawan, atau pemiliknya sendiri, BUKAN armada agregator. Makanya nggak ada akun kurir, nggak ada aplikasi kurir, nggak ada bagi hasil, dan nggak ada lacak posisi (gelombang 3, kalau perlu). Itu pembedanya dari GrabFood/GoFood.
    - **`couriers`** per outlet (nama, WA, kendaraan), semua tier, RLS tenant_id. Dikelola di Pengaturan web (`components/dashboard/courier-settings.tsx`, kolom KIRI biar dua kolomnya seimbang, permintaan Ivan). App kasir baca lewat `couriersProvider` (API, BUKAN Drift: menyerahkan ke kurir memang butuh server).
    - **`orders.delivery_status`** NULL | on_the_way | delivered | failed, SENGAJA terpisah dari `orders.status` (alasan yang sama dengan `kitchen_status`): `ready` = makanannya jadi, bukan lagi di jalan. **`courier_name` di-SNAPSHOT** waktu ditugaskan (pelajaran nama varian #26): kurir dihapus, riwayat tetap nulis siapa yang nganter. `courier_id` boleh NULL = nama ketikan sekali jalan (tetangga, ojol panggilan).
    - **Satu pintu `backend/services/delivery_dispatch.py`**: `dispatch` / `mark_delivered` / `mark_failed`. Endpoint `POST /orders/{id}/dispatch` (courier_id ATAU courier_name), `/delivered` (bukti + penerima, DUA-DUANYA opsional: kurir kehujanan nggak boleh kejebak wajib foto), `/delivery-failed`. Dispatch ditolak kalau order masih `pending` (toko belum terima).
    - **Sampai = order `completed` + COD lunas.** COD ditarik ke `order_lifecycle.settle_cod_payment`, dipakai DUA jalur yang sama-sama berarti "diterima pelanggan": tombol Selesai (`PUT status completed`) dan tombol Sampai. Kalau nambah jalur ketiga, panggil helper itu; kalau nggak, uang COD nggak pernah kelihatan di Beranda. `mark_delivered` idempoten.
    - **Gagal antar TIDAK membatalkan order dan TIDAK balikin stok**: barangnya sudah jadi dan mungkin sudah dibayar. Order tetap `preparing`, uang tetap `pending`, kasir yang memutuskan Kirim ulang (dispatch lagi, alasan gagal dibersihkan) atau Tolak (jalur cancel yang tahu cara refund). WA ke pelanggan + pemilik.
    - **Nomor kurir SENGAJA dibuka ke pelanggan** di `GET /connect/orders/{id}` (`courier_phone`) dan halaman lacak (tombol Chat kurir). Itu seluruh gunanya: pelanggan bisa nanya waktu alamatnya susah. Halaman dikunci UUID order.
    - **Ongkir terkumpul jadi baris sendiri di laba rugi dan DIHITUNG PENDAPATAN** (`finance_service._pnl_block` → `delivery_fees`). Gelombang 1 bilang "uang lewat ke kurir"; dengan kurir = orang toko itu salah: yang nganter digaji lewat beban, ongkirnya masuk kantong toko. Tetap DI LUAR `total_amount`. Baris cuma tampil kalau > 0.
    - App kasir (APK v1.6.23): kartu Pesanan Online buat antar punya alur sendiri sesudah Terima: **Serahkan ke kurir** (sheet daftar kurir + ketik nama) → **Sampai** (sheet: penerima + foto kamera opsional, unggah `/media/upload`, gagal unggah = tetap sampai tanpa foto) atau **Gagal antar** (alasan) → Kirim ulang / Tolak. `statusLabel` nulis "Diantar Budi". Tombol lama "Sedang diantar" (PUT ready) buat delivery DIHAPUS: dia cuma ngubah status tanpa tahu siapa yang bawa.
    - Halaman lacak: timeline dapat "Kurir berangkat" + "Sampai"; "Siap diantar" ikut kelar begitu ada `dispatched_at` (tanpa itu langkahnya bolong di tengah dan penanda "sedang berjalan" meleset). Antar + tunai ditulis "Bayar di tempat". Jadwal jam buka sekarang dukung rentang KEDUA per hari di UI.
    - **Kurir NGGAK BUTUH app (mig 109, pertanyaan Ivan sesudah tes: "kalau kurirnya nggak ada APK-nya gimana dia mau foto?")**. Waktu dispatch, `orders.delivery_token` acak dibuat (sekali per order, dipakai ulang saat kirim ulang) dan kurir terdaftar dapat WA `msg_courier_task` berisi link `SITE_URL/{slug}/antar/{order_id}?k=token`. Halaman `app/[slug]/antar/[id]/page.tsx`: tagihan COD besar, alamat + Buka peta + Chat pelanggan, isi pesanan, tombol Sampai (nama penerima + `<input capture="environment">` foto dari kamera browser) dan Gagal antar. Backend publik `GET/POST /connect/{slug}/antar/{order_id}(/delivered multipart | /failed json)?k=`, dikunci `delivery_dispatch.token_matches` (compare_digest), token salah = 404 (jangan bocorin order-nya ada). Efek sampingnya lewat `delivery_dispatch` yang sama dengan app kasir, event bawa `by: courier`. Foto < 1 KB di-skip diam-diam (bukan 400): Sampai nggak boleh gagal gara-gara foto. `OrderResponse.courier_task_url` diisi `_attach_payment_info` supaya app punya tombol "Link kurir" (share lewat `wa.me/?text=`) buat kurir ketikan tanpa nomor.
    - **Token pakai kunci sendiri, BUKAN UUID order**: UUID order sudah beredar di link lacak pelanggan, dan pelanggan nggak boleh bisa menandai pesanannya sendiri sampai.
    - **Route Next di bawah `/api/` WAJIB didaftarin di nginx** (`location /api/proof/` dan `/api/antar/` → :3000, di atas `location /api/` → FastAPI). Kegigit: `/api/proof/` (unggah bukti bayar pelanggan, mig 104) ternyata ketelen ke FastAPI dan balas 404 SEJAK LAHIR, baru ketahuan 5 Sep waktu nambah `/api/antar/`. Cek: `curl -X POST https://selaris.id/api/proof/<uuid>` harus balas 422 "file required" dari FastAPI lewat Next, bukan `{"detail":"Not Found"}`.
51. **SEMUA janitor WAJIB lewat `backend/tasks/lock.py`** (audit 5 Sep 2026 atas permintaan Ivan). uvicorn jalan `--workers 2` dan TIAP worker punya task supervisor sendiri, jadi setiap loop yang didaftarkan di `main.py` hidup DUA kali dan menyapu data yang sama pada detik yang sama. Cuma `online_order_timeout` yang punya kunci (dikasih 3 Sep sesudah kegigit); tujuh sisanya nggak.
    - **Akibat yang sudah kejadian di produksi**: `subscription_billing` bikin invoice langganan DOBEL. Cek "sudah ada invoice?" lolos di dua worker sebelum salah satunya insert. Ketemu 4 pasang, semuanya lahir pada MENIT YANG SAMA (Kasira Coffee 299.000 tiga bulan, Dita Coffee 99.000). Sudah di-soft-delete yang kedua dengan catatan di `notes`. `stale_order_cleanup` lebih senyap lagi: dia membatalkan order yatim dan MENGEMBALIKAN STOK, dua worker = stok balik dua kali tanpa jejak.
    - Pola: `async with single_flight("nama", ttl=N) as boleh: if not boleh: return <hasil kosong>`. **TTL harus lebih pendek dari interval loop** (biar siklus berikutnya nggak ikut kekunci) dan lebih panjang dari lama satu siklus. Kunci dibiarkan kedaluwarsa sendiri, TIDAK dihapus di akhir: worker yang mati di tengah tetap melepas kunci setelah TTL.
    - Redis mati = janitor tetap jalan (lebih baik dobel daripada mati total). Yang berisiko punya lapis kedua sendiri: `FOR UPDATE SKIP LOCKED` di `online_order_timeout`, cek idempotensi di `subscription_billing`.
    - Sudah dipasang di: payment_reconciliation (280s), shift_cutoff (570s), stale_order_cleanup (3500s), sync_idempotency_cleanup (3600s), kg_price_event (280s), subscription_billing (3500s), grace_period (3500s). `platform_aggregation` (4 loop agregasi, 6 sampai 24 jam) SENGAJA belum: dia menulis tabel agregat yang di-overwrite, bukan menambah baris. Kalau nanti dia mulai insert, kasih kunci.

52. **Janitor `payment_reconciliation` JANGAN nyentuh channel `manual`** (fix 5 Sep 2026, kegigit order Ivan #5467). Aturan lama "non-QRIS pending > 10 menit = expired" lahir waktu tunai selalu lunas seketika di kasir. Sejak COD, tunai antar SENGAJA pending sampai kurir sampai, dan QRIS statis/transfer sengaja pending sampai kasir memastikan bukti. Janitor meng-expire COD yang masih di jalan (menit ke-14 dari perjalanan 1 jam), lalu `settle_cod_payment` yang cuma mau `pending` melewatinya: order selesai, uang HILANG dari Beranda. Sekarang query kandidat mengecualikan `channel='manual'` (NULL tetap ikut = data lama), dan `settle_cod_payment` juga memulihkan `expired`. Prinsipnya: **yang manusia selesaikan, jangan diselesaikan mesin.** Metode bayar manual baru wajib `channel='manual'`, itu yang melindunginya.

50. **Tiga kasir satu pesanan: kunci barisnya, dan jangan tampilkan merah (5 Sep 2026, mig 110).** Pertanyaan Ivan: "kalau kasir ada 3 pegang aplikasi terus pesanan online masuk, bentrok nggak?" Jawabannya dulu: IYA.
    - **`_load_order_full(db, id, lock=True)` = `SELECT ... FOR UPDATE`.** WAJIB di semua endpoint yang MEMUTUSKAN: `/accept`, `/reject`, `/dispatch`, `/delivered`, `/delivery-failed`, `/kitchen-status`. Tanpa itu tiga permintaan bersamaan sama-sama membaca `pending`, sama-sama lolos guard, dan pelanggan dapat TIGA WA. Diverifikasi: 3 accept bersamaan → 1x200 + 2x409, satu event `order.accepted`, `row_version` 1. `selectinload` aman bareng FOR UPDATE (koleksi item ditarik query terpisah, yang dikunci cuma baris order).
    - **Kalah cepat = 409, BUKAN 400.** 400 dipakai buat "kamu salah" (misal serahkan ke kurir sebelum diterima); 409 buat "kolega sudah menangani". App membedakannya lewat `ActionResult.conflict` (`online_orders_provider`): daftar dimuat ulang, snackbar BIRU, bukan merah. Dulu kasir kedua lihat "Pesanan ini sudah diproses" warna merah dan mengira sistemnya rusak.
    - **`orders.accepted_by` + `accepted_by_name`** di `OrderResponse` (diisi `_attach_payment_info` lewat satu query nama). Pesan bentrok jadi "sudah diterima Budi", dan kartu di SEMUA HP nulis "Disiapkan · Budi". Data lama `accepted_by` NULL, wajar.
    - **Dapur beda perlakuan**: dua tablet menekan Siap bareng dua-duanya dibalas 200 (dapur nggak perlu tahu siapa duluan), tapi status yang sama = no-op lebih awal, jadi cuma satu `kitchen.ready` dan satu WA "pesanan siap".
    - `PUT /orders/{id}/status` nggak diubah: dia sudah aman lewat `WHERE row_version = :expected ... RETURNING` (optimistic lock, Rule #30).
    - Kabar antar-HP tetap lewat SSE `orders:{outlet_id}` (tiap event = tarik ulang daftar), polling 30 detik sebagai jaring. Kunci baris ngurus kebenaran data; SSE ngurus layar yang lain ikut segar.
    - **Nggak bentrok dengan CRDT sync** (diaudit 5 Sep atas pertanyaan Ivan): `process_table_sync` (`services/sync.py`) juga pakai `with_for_update()` per baris, jadi dua-duanya kunci baris Postgres biasa yang saling nunggu, bukan dua mekanisme yang berebut. Urutan kunci konsisten (order dulu, baru payment) di kedua jalur, jadi nggak ada siklus deadlock. Endpoint yang dikunci itu aksi ONLINE atas order storefront; sync cuma mendorong baris `isSynced=false` yang lahir di HP, dan baris yang ditarik dari server ditandai `isSynced: true`, jadi order storefront nggak pernah didorong balik. Kalau pun ada salinan basi didorong, `financial_strict` (completed/cancelled = server menang) plus LWW `updated_at`+`row_version` (accept selalu menaikkan dua-duanya) bikin server menang. Pull `_row_to_dict` mengirim SEMUA kolom model ke HP, tapi `_applyServerChanges` memilih key secara eksplisit, jadi kolom baru (courier_*, delivery_*, accepted_by) cuma diabaikan. **`delivery_token` disaring lewat `_SYNC_SKIP_COLUMNS`**: rahasia link kurir nggak boleh nyangkut di Drift setiap HP. Kalau nambah kolom rahasia di model yang ikut sync, taruh di situ.

    - **BELUM**: lacak posisi live (halaman kurir ini tempat alaminya: tinggal `navigator.geolocation.watchPosition` + endpoint posisi), rekap per kurir (data `courier_id` di orders sudah ada, tinggal query), kurir dari app kasir (sekarang cuma web), FCM.

53. **Notifikasi push FCM (5 Sep 2026) — folder `android/` NGGAK ADA di repo, jadi semua berkas Firebase dipasang di CI.** Latar: kabar "pesanan online masuk" cuma lewat SSE `orders:{outlet_id}` yang HANYA hidup selama app kasir dibuka, plus WA ke pemilik. HP terkunci di saku = kasir buta, padahal batas konfirmasinya 10 menit.
    - **Tabel `devices` sudah ada sejak mig 007** lengkap dengan `fcm_token`, tapi nol model, nol route, nol baris sampai hari ini. Persis kasus `product_variants` (#26) dan purchasing (#29). **Cek `\d` di DB sebelum bikin tabel baru.** Nol migrasi baru buat fitur ini.
    - **Pakai FCM HTTP v1, bukan server key legacy** (`fcm.googleapis.com/fcm/send` sudah dimatikan Google Juni 2024). `backend/services/fcm.py` nandatangani JWT RS256 pakai `python-jose`, nukernya ke OAuth2 access token, lalu `POST /v1/projects/{id}/messages:send`. **Nggak nambah dependensi**: `firebase-admin` itu SDK sinkron yang bakal ngeblok event loop (Rule #9) dan narik grpc. Token akses di-cache Redis `fcm:access_token` (umur dikurangi 2 menit).
    - **Satu pintu, sama seperti WA: `online_orders.wa_owner`.** Dia sekarang manggil `push_app()` DULU baru WA. Kanal baru tetap ditambah di situ, bukan di pemanggil. Pemanggil cuma nambah `title=` dan `data={"route": ...}`; `route` itu yang bikin ketukan notifikasi mendarat di layar yang bener.
    - **Kunci server kosong = fitur mati diam-diam, bukan error.** `fcm.enabled()` False bikin semua jalur balik 0 dan WA tetap jalan seperti sebelumnya. Ini yang bikin kodenya aman di-deploy sebelum proyek Firebase-nya jadi.
    - **Channel `pesanan_online` ditulis di TIGA tempat dan harus sama persis**: `CHANNEL_ID` di `services/fcm.py`, `_channelId` di `push_service.dart`, dan meta-data `default_notification_channel_id` di AndroidManifest (ditulis workflow CI). Beda satu huruf = Android jatuh ke channel cadangan yang nggak nongol di layar. **Importance nempel PERMANEN ke channel begitu dibikin**: ganti nilainya di kode nggak ngaruh sampai app di-uninstall, makanya sekali bikin langsung `high`.
    - **CI: `flutter create` bikin ulang `android/` dari nol tiap build** (folder itu nggak ada di repo, lihat langkah "Recreate Platform Directories"). Jadi `google-services.json` DAN patch gradle harus jalan di workflow, sesudah langkah itu, sama seperti patch manifest dan ikon aplikasi. Rahasianya `GOOGLE_SERVICES_JSON` di GitHub Secrets; kalau kosong, langkahnya dilewati dan APK tetap kebentuk tanpa push (main tetap hijau).
    - **Gradle-nya Kotlin DSL** (`settings.gradle.kts` + `app/build.gradle.kts`), bukan Groovy. Yang dipatch: plugin `com.google.gms.google-services` 4.4.3 (`apply false` di settings, dipakai di app) plus `isCoreLibraryDesugaringEnabled = true` + `coreLibraryDesugaring("com.android.tools:desugar_jdk_libs:2.1.4")` yang disyaratkan `flutter_local_notifications` 22.x. **Sudah dibuktikan build lokal** dengan `google-services.json` boneka: AGP 9.0.1 + google-services 4.4.3 + desugaring lolos, APK 72 MB, manifest hasil merge bawa `FirebaseMessagingService` + channel + POST_NOTIFICATIONS. Kalau nambah plugin Android lagi, buktiin lokal dulu dengan cara yang sama — satu putaran CI itu 15 menit.
    - **Foreground ditampilkan APP, bukan sistem.** FCM nggak nampilin apa-apa waktu app lagi dibuka, cuma nyerahin datanya. Makanya `flutter_local_notifications` yang nampilin. Tetap perlu walau ada SSE: SSE bisa putus, dan kasir bisa lagi buka layar lain.
    - **Identitas perangkat itu `fcm_token`, BUKAN pasangan user+outlet.** Satu HP dipakai kasir gantian = token tetap satu; satu akun pegang tiga HP = ketiganya harus bunyi. Upsert di `POST /devices/register` jalan di sesi TERPISAH tanpa RLS (`SET LOCAL app.current_tenant_id = ''`), karena satu HP bisa login ke dua tenant (akun demo Ivan dan akun tokonya) dan baris tenant lama nggak kelihatan dari dalam RLS tenant baru. Kalau kelewat, tenant lama terus ngirim notifikasi ke HP yang sudah pindah tangan.
    - **Logout WAJIB nyabut token SEBELUM `SessionCache.clear()`** — endpointnya butuh Authorization. Token yang ditolak Firebase (app di-uninstall, data dibersihkan) dicabut sendiri sama `notify_outlet` waktu balasannya 404/UNREGISTERED.
    - `POST /devices/test-push?outlet_id=` ada karena push itu susah didiagnosa: nggak bunyi bisa berarti kunci server, token nggak kedaftar, izin ditolak, atau channel salah. Tombol itu yang misahin "server nggak ngirim" dari "HP nggak nampilin".
    - Kunci dipasang lewat `scripts/pasang_fcm.sh <service-account.json>` (ngubah newline kunci privat jadi `\n` harfiah; ditempel mentah, parser .env patah dan SEMUA env sesudahnya hilang). **Env baru = `compose build backend && up -d --no-deps backend`**, `docker restart` nggak muat ulang env, dan recreate bikin berkas `docker cp` hilang (gotcha #9).
    - **BELUM**: app Dapur belum didaftarkan (`device_type` `dapur` sudah ada di ENUM, `notify_outlet` defaultnya cuma `kasir` + `owner`), notifikasi ke pelanggan, dan halaman "perangkat terdaftar" di Pengaturan.

54. **Tablet itu perangkat UTAMA (v1.6.27, 6 Sep 2026), dan app ini lahir phone-first.** Keputusan Ivan menjelang launch: coffee shop pakai tablet, UMKM kecil pakai HP. Tes di tablet Huawei ngeluarin tiga bug yang akarnya beda-beda, jangan digebyah uyah.
    - **`shortestSide` itu properti PERANGKAT, bukan orientasi.** Angkanya nggak berubah waktu tablet diputar, jadi `shortestSide >= 600` bikin tablet BERDIRI tetap dikasih layout dua kolom rancangan landscape: keranjang keremet ~240px, produk dipaksa 4 kolom di ruang 560px. Pakai `size.width` (ambang 900) supaya tablet berdiri jatuh ke layout HP yang udah mateng. Nol layout baru ditulis.
    - **Landscape masalahnya TINGGI, bukan lebar.** Tablet tidur itu lebar tapi pendek (~800px; 1024x600 malah ~550 sesudah inset dialog), dan keyboard makan ~300px lagi. Semua angka `height * 0.7` / `0.85` di modal dulu dikalibrasi di HP jangkung.
    - **JANGAN patok ukuran modal dengan angka mati.** `payment_modal` punya `SizedBox(width: 760, height: 580)`; kotak yang nggak bisa mengecil = isinya kepotong, dan yang paling bawah persis tombol Bayar. Ikat ke `size - viewInsets.bottom`, isinya digulung, dan **tombol aksi utama taruh DI LUAR area gulung** biar selalu kelihatan. Pilih layout dari `size.height` juga; pakai `size`, BUKAN sisa ruang sesudah keyboard, biar layoutnya nggak loncat waktu kasir lagi ngetik nominal.
    - **Halaman yang di-`Navigator.push` WAJIB punya tombol kembali di layar.** Route yang di-push nutupin bar navigasi bawah dashboard. Tanpa AppBar dan tanpa tombol, jalan keluarnya cuma gestur back Android yang di banyak tablet nggak kelihatan. `settings_page` kegigit persis begini: header cuma `Text('Pengaturan')`. Halaman tanpa AppBar juga wajib `SafeArea` sendiri (sama alasannya dengan gotcha #38).
    - **Batasi lebar isi di layar lebar** lewat `core/widgets/lebar_konten.dart` (`LebarKonten`, maks 720, nggak ngapa-ngapain di layar sempit). Halaman yang punya pita header berwarna DI DALAM body dibungkus DUA titik terpisah (isi header + isi daftar); pitanya sendiri tetap selebar layar, kalau ikut dipersempit warnanya berhenti di tengah dan zona status bar belang. Halaman ber-AppBar boleh dibungkus utuh.
    - **Kita buta soal hardware merchant.** `devices.device_name` diisi nama outlet (`push_service.dart`), padahal itu satu-satunya kolom yang bisa nyimpen merek/model. Selama masih begitu, "merchant pakai tablet apa HP" cuma tebakan.
    - **BELUM device test.** Yang diperbaiki itu hitungan ruang yang jelas salah. Satu angka ditebak dan udah dikasih catatan di kodenya: `childAspectRatio` buat 3 kolom di tablet berdiri.

55. **Izin runtime: jangan bikin gerbang sendiri di depan panggilan OS (fix 11 Sep 2026, ketahuan waktu audit launch).** `printer_service.dart` dulu nolak lanjut kalau salah satu dari bluetoothScan/bluetoothConnect/locationWhenInUse nggak `granted`. Dua kelompok merchant kekunci sekaligus dan dua-duanya nggak kelihatan dari kode:
    - **Android 12+**: daftar printer yang sudah di-pair (`pairedBluetooths`, bonded list) cuma butuh BLUETOOTH_CONNECT. Kita nggak pernah discovery perangkat baru, jadi **lokasi nggak ada hubungannya sama printer**. Merchant yang nolak lokasi = nggak bisa cetak sama sekali, dan nggak ada jalan lain di app.
    - **Android 11 ke bawah**: `Permission.bluetoothScan` **selalu** balik `denied`. permission_handler nggak nambah nama izin apa pun buat BLUETOOTH_SCAN di bawah SDK 31 (`PermissionUtils.java`, case `PERMISSION_GROUP_BLUETOOTH_SCAN` dibungkus `if SDK_INT >= S`), dan daftar nama kosong dibaca DENIED (`PermissionManager.java:368`). Jadi syarat "semua harus granted" nggak akan pernah kepenuhan. **Cek status izin Bluetooth buat MEMUTUSKAN alur = jebakan**, mirip `/cek` sebelum kirim di gotcha #46.
    - **Polanya**: minta izinnya, lalu tetap panggil API aslinya. Yang menegakkan OS, bukan tebakan kita. Gagal = pesan tegas + **tombol Buka Pengaturan** (`openAppSettings()`), karena sesudah user nekan Tolak dua kali Android nggak pernah nanya lagi dari dalam app. Izin Bluetooth tetap WAJIB (keputusan Ivan: "bukan bahan nego"); yang dilonggarin cuma lokasi.
    - Daftar kosong WAJIB ada tulisannya. Dulu tombol selesai berputar lalu senyap, kasir nggak tahu printernya harus di-pair dulu di Pengaturan HP.
    - **`PrinterState.copyWith(error: null)` TIDAK ngapus error** (`error ?? this.error`). Mau bersihin, lewat `clearError: true`.

56. **Rilis Play Store: tanda tangan asli + DUA applicationId (11 Sep 2026).** Sampai v1.6.28 semua rilis ditandatangani **DEBUG KEY** (`CN=Android Debug`, kebukti dari APK produksi), dan POS + Dapur keluar dengan applicationId yang SAMA (`com.kasira.kasira_kasir`). Sideload jalan, Play nolak mentah-mentah.
    - **Sekarang**: `flutter create --org com.selaris --project-name selaris_pos`, `namespace` tetap `com.selaris.selaris_pos` (jangan diubah, itu paket kode + lokasi MainActivity.kt), dan **applicationId dipatch per app di dalam langkah build**: `com.selaris.pos` lalu `com.selaris.dapur`. Label manifest Dapur ikut dipatch jadi "Selaris Dapur", kalau nggak dua ikon di satu tablet sama-sama nulis "Selaris".
    - **Keystore diinject dari secret** (`ANDROID_KEYSTORE_BASE64` + password + alias), persis pola `google-services.json`, karena `android/` dibikin ulang tiap build. Aslinya di `/etc/selaris/upload-keystore.jks` (chmod 600, password di `/etc/selaris/keystore-password.txt`). Secret kosong = balik ke debug key + peringatan, main tetap hijau. Ada langkah **Verifikasi tanda tangan** yang MEMATIKAN build kalau APK masih bau debug key: lebih baik ketahuan di CI daripada di Play Console.
    - **google-services.json WAJIB memuat KEDUA applicationId.** Plugin google-services nolak build kalau paketnya nggak terdaftar. Langkah "Pasang google-services.json" sekarang ngecek itu: kurang satu = berkasnya dihapus, Firebase dilewati, APK tetap kebentuk **tanpa push** (bukan build yang mati). Perbarui lewat `scripts/pasang_google_services.sh`.
    - **Desugaring DIPISAH dari langkah Firebase.** Dulu satu langkah bersyarat `if firebase ada`; padahal desugaring itu syarat `flutter_local_notifications` 22.x, jadi build tanpa Firebase langsung mati.
    - **`kasir_app/android/` sekarang di-.gitignore** bareng `*.jks` dan `key.properties`. Waktu build lokal folder itu berisi keystore + password rilis, dan satu `git add -A` cukup buat ngebocorinnya.
...
41. **Shift/sesi kas itu OTOMATIS sejak 3 Sep 2026 (mig 097–098). Jangan pernah lagi nulis kode yang nolak transaksi dengan "buka shift dulu".** Latar: 25 dari 27 shift produksi terbuka >1 hari, satu di antaranya 4 bulan; tiga jalur transaksi nyari shift dengan caranya sendiri sampai Beranda nulis "Shift aktif" tapi simpan ke meja ditolak. Rujukan desain: cash drawer Toast (business-day cutoff 04.00, paused drawer, blind close, lockdown opsional).
    ...
    - **Sesi yang sudah `closed` tapi `counted_at IS NULL` WAJIB tetap bisa dihitung lewat `POST /shifts/{id}/close`** (fix 4 Sep malam). Dulu endpoint itu nolak semua yang `closed` dengan "Shift sudah ditutup", jadi tombol Hitung di daftar "Kas belum dihitung" (yang isinya justru hasil auto_cutoff) selalu 400 dan sesi itu nggak pernah bisa dihitung. Sekarang: `closed` + belum dihitung = `shift_service.count_closed_shift` (cuma isi `ending_cash`, `counted_at`, selisih; status/end_time/closed_reason nggak disentuh, audit `COUNT_SHIFT`); `closed` + sudah dihitung = 400 "Kas sesi ini sudah dihitung". Nggak butuh APK baru.
