...
26. **Varian produk (Hot/Ice, size, level gula) BELUM ADA — tabelnya doang yang ada.** `product_variants` (migration `014`) + `order_items.product_variant_id` + `order_items.modifiers` (JSONB) udah ada di DB, tapi **nol** API CRUD, nol schema, nol sync, nol UI dashboard/Flutter/storefront, dan nol baris di produksi. `CartItem.notes` di Flutter juga udah kecetak di struk tapi **nggak ada UI buat ngisinya**. Jadi merchant kepaksa bikin dua produk terpisah "Kopi Hot" + "Kopi Ice" → resep & stok kembar. Kalau mau dibangun: varian nyentuh resep & HPP juga (Ice butuh es batu, Large susu lebih banyak) — jangan cuma nambah penyesuaian harga tanpa mikirin 6+ stock code path.
