...
49. **Delivery gelombang 2: kurir = ORANG TOKO (5 Sep 2026, mig 108).** Keputusan Ivan: yang nganter itu anak pemilik, karyawan, atau pemiliknya sendiri, BUKAN armada agregator. Makanya nggak ada akun kurir, nggak ada aplikasi kurir, nggak ada bagi hasil, dan nggak ada lacak posisi (gelombang 3, kalau perlu). Itu pembedanya dari GrabFood/GoFood.
    ...
    - **Janitor `payment_reconciliation` JANGAN nyentuh channel `manual`** (fix 5 Sep 2026, kegigit order Ivan #5467). Aturan lama "non-QRIS pending > 10 menit = expired" lahir waktu tunai selalu lunas seketika; sejak COD, tunai antar SENGAJA pending sampai kurir sampai, dan QRIS statis/transfer sengaja pending sampai kasir memastikan bukti. Janitor meng-expire COD yang masih di jalan, lalu `settle_cod_payment` (yang cuma mau `pending`) melewatinya: order selesai, uang hilang dari Beranda. Sekarang query kandidat mengecualikan `channel='manual'` (NULL tetap ikut = data lama), dan `settle_cod_payment` juga memulihkan `expired`. Prinsipnya: **yang manusia selesaikan, jangan diselesaikan mesin.** Kalau nambah metode bayar manual baru, pastikan channel-nya `manual`, itu yang melindunginya dari janitor.
